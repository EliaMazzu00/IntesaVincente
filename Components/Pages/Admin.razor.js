...
// campo vengono ignorati, altrimenti configurare la partita farebbe scattare
// giudizi a raffica.
...
/** I tasti che comandano qualcosa in regia. */
const SHORTCUT_KEYS = ["g", "s", "p", "d", "enter", " "];
...
function onKeyDown(event) {
    if (!component || event.ctrlKey || event.altKey || event.metaKey || event.repeat) {
        return;
    }
    ...
    if (!SHORTCUT_KEYS.includes(event.key.toLowerCase())) {
        return;
    }
